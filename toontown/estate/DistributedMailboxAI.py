...
class DistributedMailboxAI(DistributedObjectAI):
    ...
    def avatarEnter(self):
        ...
        if len(av.mailboxContents):
            ...
            # The avatar has seen their items now.
    ...
    def acceptInviteMessage(self, context, inviteKey):
        ...
        if self.user != avId:
            ...
            # it's possible the party was cancelled right before they accepted
    ...
    def markInviteReadButNotReplied(self, inviteKey):
        # Mark the invite as read but not replied in the database.
        avId = self.air.getAvatarIdFromSender()
        validInviteKey = False
        toon = simbase.air.doId2do.get(avId)
        for invite in toon.invites:
            if invite.inviteKey == inviteKey and invite.status == InviteStatus.NotRead:
                validInviteKey = True
                break
        if validInviteKey:
    ...
    def rejectInviteMessage(self, context, inviteKey):
        ...
        if self.user != avId:
            ...
            # hand it off to party manager
            # it's possible the party was cancelled right before they rejected
