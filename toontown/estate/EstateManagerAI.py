...
class LoadHouseOperation(FSM):
    ...
    def start(self):
        ...
        if self.avatar is None:
            ...
            taskMgr.doMethodLater(0.0, self.__demandHouseTask, 'makeBlankHouse-%s' % id(self))
            return
    ...
    def __demandHouseTask(self, task):
...
class LoadEstateOperation(FSM):
    ...
    def __handleQueryAccount(self, dclass, fields):
        ...
        if dclass != self.mgr.air.dclassesByName['AstronAccountAI']:
            self.mgr.notify.warning('Account %d has non-account dclass %s!' % (self.accId, str(dclass)))
            self.demand('Failure')
            return
    ...
    def __handleQueryAvatar(self, dclass, fields, index):
        ...
        if dclass != self.mgr.air.dclassesByName['DistributedToonAI']:
            self.mgr.notify.warning(
                'Account %d has avatar %d with non-Toon dclass %s!' % (self.accId, self.avIds[index], str(dclass)))
            self.demand('Failure')
            return
    ...
    def __handleEstateCreated(self, estateId):
        ...
        self.mgr.air.dbInterface.updateObject(self.mgr.air.dbId, self.accId, self.mgr.air.dclassesByName['AstronAccountAI'],
                                              {'ESTATE_ID': estateId})
    ...
    def enterLoadPets(self):
        ...
        if not self.petOperations:
            taskMgr.doMethodLater(0, self.__finishPetLoadTask, 'noPets-%s' % id(self))

    def __finishPetLoadTask(self, task):
