...
class NameShop(StateData.StateData):
    ...
    if not __astron__:
        def handleSetNamePatternAnswerMsg(self, di):
            ...
            if returnCode == 0:
                ...
                avAccessories = self.getAccessoryData()
    ...
    # TODO: It's weird that this avatar creation code is in NameShop.
    def serverCreateAvatar(self, skipTutorial = False):
        self.notify.debug('serverCreateAvatar')
        style = self.toon.getStyle()
        self.newDNA = style.makeNetString()
        self.newAccessories = self.getAccessoryData()
        if skipTutorial:
            self.requestingSkipTutorial = True
        else:
            self.requestingSkipTutorial = False
        if not self.avExists or self.avExists and self.avId == 'deleteMe':
            accessories = [self.toon.hat, self.toon.glasses, self.toon.backpack, self.toon.shoes]
            messenger.send('nameShopCreateAvatar', [style, accessories, '', self.index])
            if __astron__:
    ...
    def getAccessoryData(self):
        return Toon.makeAccessoryNetString(self.toon.hat, self.toon.glasses, self.toon.backpack, self.toon.shoes)
        ...
        def handleCreateAvatarResponseMsg(self, di):
            ...
            if returnCode == 0:
                ...
                if self.nameAction == 0:
                    self.toon.setName(self.names[0])
                    newPotAv = PotentialAvatar.PotentialAvatar(self.avId, self.names, self.newDNA, self.newAccessories, self.index, 1)
                    self.avList.append(newPotAv)
                    self.doneStatus = 'done'
                    self.storeSkipTutorialRequest()
                    messenger.send(self.doneEvent)
